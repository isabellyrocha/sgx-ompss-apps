...
void save_convolutional_weights_binary(layer l, int fp)
{
    ...
    int i, j, k;
    ocall_fwrite(l.biases, sizeof(float), l.n);
    if (l.batch_normalize)
    {
        ocall_fwrite(l.scales, sizeof(float), l.n);
        ocall_fwrite(l.rolling_mean, sizeof(float), l.n);
        ocall_fwrite(l.rolling_variance, sizeof(float), l.n);
    }
    for (i = 0; i < l.n; ++i)
    {
        ...
        if (mean < 0)
            mean = -mean;
        ocall_fwrite(&mean, sizeof(float), 1);
        for (j = 0; j < size / 8; ++j)
        {
...
void save_convolutional_weights(layer l, int fp)
{
    ...
    ocall_fwrite(l.weights, sizeof(float), num);
}

void save_batchnorm_weights(layer l, int fp)
{

    ocall_fwrite(l.scales, sizeof(float), l.c);
    ocall_fwrite(l.rolling_mean, sizeof(float), l.c);
    ocall_fwrite(l.rolling_variance, sizeof(float), l.c);
}

void save_connected_weights(layer l, int fp)
{

    ocall_fwrite(l.biases, sizeof(float), l.outputs);
    ocall_fwrite(l.weights, sizeof(float), l.outputs * l.inputs);
    if (l.batch_normalize)
    {
        ocall_fwrite(l.scales, sizeof(float), l.outputs);
        ocall_fwrite(l.rolling_mean, sizeof(float), l.outputs);
        ocall_fwrite(l.rolling_variance, sizeof(float), l.outputs);
    }
}
...
void save_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    int minor = 2;
    int revision = 0;
    ocall_fwrite(&major, sizeof(int), 1);
    ocall_fwrite(&minor, sizeof(int), 1);
    ocall_fwrite(&revision, sizeof(int), 1);
    ocall_fwrite(net->seen, sizeof(size_t), 1);
    ...
    for (i = 0; i < net->n && i < cutoff; ++i)
    {
        ...
        if (l.type == LOCAL)
        {
            ...
            ocall_fwrite(l.biases, sizeof(float), l.outputs);
            ocall_fwrite(l.weights, sizeof(float), size);
        }
    }
    ocall_close_file();
}
...
{
    ocall_fread(l.biases, sizeof(float), l.outputs);
    ocall_fread(l.weights, sizeof(float), l.outputs * l.inputs);
    if (transpose)
    {
    ...
    {
        ocall_fread(l.scales, sizeof(float), l.outputs);
        ocall_fread(l.rolling_mean, sizeof(float), l.outputs);
        ocall_fread(l.rolling_variance, sizeof(float), l.outputs);
...
{
    ocall_fread(l.scales, sizeof(float), l.c);
    ocall_fread(l.rolling_mean, sizeof(float), l.c);
    ocall_fread(l.rolling_variance, sizeof(float), l.c);
}

void load_convolutional_weights_binary(layer l, int fp)
{
    ocall_fread(l.biases, sizeof(float), l.n);
    if (l.batch_normalize && (!l.dontloadscales))
    {
        ocall_fread(l.scales, sizeof(float), l.n);
        ocall_fread(l.rolling_mean, sizeof(float), l.n);
        ocall_fread(l.rolling_variance, sizeof(float), l.n);
    }
    int size = l.c * l.size * l.size;
    int i, j, k;
    for (i = 0; i < l.n; ++i)
    {
        float mean = 0;
        ocall_fread(&mean, sizeof(float), 1);
        for (j = 0; j < size / 8; ++j)
        {
            int index = i * size + j * 8;
            unsigned char c = 0;
            ocall_fread(&c, sizeof(char), 1);
            for (k = 0; k < 8; ++k)
            {
                if (j * 8 + k >= size)
                    break;
...
void load_convolutional_weights(layer l, int fp)
{
    ...
    ocall_fread(l.weights, sizeof(float), num);
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    int fp = 0;//ocall_open_file(filename, O_RDONLY);;
    //printf("Done with ocall open_file..\n");
    int major;
    int minor;
    int revision;
    //printf("\n MINOR VALUE: %d", minor);
    ocall_fread(&major, sizeof(int), 1);
...
/*
    ...
    printf("\nMAJOR VALUE: %d", major);
    printf("declaring new minoe");
    minor = 0;
    printf("after  minor");
  // printf("\n MINOR VALUE: %d", minor);

    printf("before transpose");
    int transpose = 0;//(major > 1000) || (minor > 1000);
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    ocall_fread(&major, sizeof(int), 1);
    ocall_fread(&minor, sizeof(int), 1);
    ocall_fread(&revision, sizeof(int), 1);

    if ((major * 10 + minor) >= 2 && major < 1000 && minor < 1000)
    {
        ocall_fread(net->seen, sizeof(size_t), 1);
    }
    else
    {
        int iseen = 0;
        ocall_fread(&iseen, sizeof(int), 1);
        *net->seen = iseen;
    }
    int transpose = (major > 1000) || (minor > 1000);
    ...
    for (i = start; i < net->n && i < cutoff; ++i)
    {
        ...
        if (l.type == LOCAL)
        {
            ...
            int size = l.size * l.size * l.c * l.n * locations;
            ocall_fread(l.biases, sizeof(float), l.outputs);
            ocall_fread(l.weights, sizeof(float), size);
        }
    }
    ocall_close_file();
}
